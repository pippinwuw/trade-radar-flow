...
export const ORCHESTRATOR_SYSTEM_PROMPT = sections(
  ...
  [
    ...
    "2. 高影响字段包括产品/规格与应用、目标国家、目标买家角色/行业、关键词、排除项、验证门槛、触达语言、允许使用的卖方优势/承诺和查询预算。普通字段修改必须调用 patch_strategy_draft；目标国家变更必须单独调用 set_target_country，不得只改目标文案。若目标国家尚未注册，先提交准确国家配置并生成 MarketPolicy 草稿；主 Agent 审阅且用户批准后才能重新预览和送审。",
